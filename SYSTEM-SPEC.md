...
> 更新时间：2026-09-14
> 文档状态：已归档系统行为规格
> 当前归档：`changes/archive/2026-09-14-local-avatar-upload/`
> 规格依据：`changes/archive/2026-09-14-local-avatar-upload/spec.md`
...
| 用户头像 | 已实现基线 | 用户资料支持本地上传头像，侧边栏、工单、在线咨询展示头像并提供兜底 |
...
- 所有登录角色都可以在个人资料中上传、修改或清除自己的本地头像。
- 头像读取地址是用户公开资料字段，可出现在当前用户响应、用户列表、工单 requester、assignee、message author 和 audit actor 摘要中。
- 头像上传只接受 PNG、JPG/JPEG 和 WebP，文件大小不得超过 2 MB，并必须通过扩展名、MIME 和 magic number 校验。
- 后端保存头像到本地隔离目录，并生成 `avatar_url` 作为公开图片读取地址；用户不能手填外链 URL。
- 头像读取地址可由浏览器 `<img>` 直接访问，不要求 Bearer token；上传和清除头像必须要求登录。
- 前端在侧边栏用户菜单、工单列表客户/负责人列、工单时间线、在线咨询用户消息中展示用户头像。
- 用户头像缺失或图片加载失败时，前端必须显示姓名或邮箱首字母兜底。
- AI Agent 在在线咨询中使用固定系统头像；头像兜底必须可见显示 `AI`。
- 本能力不包含图片裁剪、对象存储、CDN 或 Workspace 级独立头像。
...
- 手填外链头像 URL、图片裁剪、对象存储、CDN 和 Workspace 级独立头像。
