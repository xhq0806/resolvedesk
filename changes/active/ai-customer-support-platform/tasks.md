...
- [x] 【基线验证】(全栈) 验证工程整理结果 `← depends: 【模板清理】`
  ...
  - 涉及文件：工程基线全部变更文件
  - 预期结果：`git check-ignore`、`git diff --check`、`docker compose config --quiet`、`uv lock --check`、目标文件 prek 检查和前端构建通过。
...
- [ ] 【数据迁移】(后端) 编写角色与工单迁移 `← depends: 【工单模型】`
...
- [ ] 【客户端生成】(前端) 更新 OpenAPI 客户端 `← depends: 【后端契约】`
  - 目标：从稳定后端契约生成 User 与 Ticket 客户端。
  - 涉及文件：`frontend/openapi.json`、`frontend/src/client/`
  - 预期结果：前端无第二套手写业务 API。
- [ ] 【认证路由】(前端) 实现角色 guard 与缓存隔离 `← depends: 【客户端生成】`
...
- [ ] 【工单详情】(前端) 实现共享详情和时间线
  ...
  - 预期结果：后端裁剪结果按角色正确展示。
- [ ] 【工单操作】(前端) 实现状态属性与 Admin 分派 `← depends: 【工单详情】`
