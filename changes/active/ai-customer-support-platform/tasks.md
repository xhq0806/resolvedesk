...
- [x] 【全栈联调】（全栈）验证三角色业务闭环 `-> depends: 【前端测试】`
  - 目标：验证创建、接手或分派、回复、等待、重开、解决、关闭和删除。
  - 涉及文件：全栈业务入口
  - 预期结果：三角色完整流程可重复执行。
- [x] 【安全回归】（全栈）验证隔离与并发不变量 `-> depends: 【全栈联调】`
  - 目标：验证内部备注隔离、原子接手和最后活跃 Admin。
  - 涉及文件：后端安全测试、前端 E2E
  - 预期结果：关键权限和并发不变量通过。
- [x] 【发布验收】（全栈）执行完整质量门控 `-> depends: 【安全回归】`
...
- 前端角色化 Playwright E2E：`56 passed`；新增业务闭环场景：`2 passed`
- 后端测试：`170 passed`
- 后端质量门控：`ruff check`、`ruff format --check`、`mypy`、`ty` 全部通过
- 前端质量门控：生产构建和 Biome lint 全部通过
- OpenAPI：后端契约重新导出，正式客户端已同步 `UserSummary.is_active`
- Docker Compose：配置与启动健康检查通过
- 数据库：迁移升级和降级演练通过
