...
def can_view_ticket(
    actor: User, ticket: Ticket, *, role: UserRole | None = None
) -> bool:
    ...
    if effective_role is UserRole.AGENT:
        return ticket.assignee_id is None or ticket.assignee_id == actor.id
    return effective_role is UserRole.ADMIN
...
def can_view_internal_notes(
    ...
    if not can_view_ticket(actor, ticket, role=effective_role):
        return False
    if effective_role is UserRole.AGENT:
...
def can_reply_publicly(
    ...
    if ticket.status is TicketStatus.CLOSED or not can_view_ticket(
        actor, ticket, role=effective_role
    ):
        return False
    if effective_role is UserRole.CUSTOMER:
        return ticket.requester_id == actor.id
